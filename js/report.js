/**
 * صياغة وتصدير التقارير النصية و PDF (Reporting & Exporting)
 */
...
function formatDurationText(hours, minutes) {
    ...
    const hText = h > 0 ? formatHoursText(h) : '';
    const mText = m > 0 ? `${m} دقيقة` : '';
    if (hText && mText) return `${hText} و${mText}`;
...
function buildPdfHtml(sessionFields, studentsList) {
    const { date, halaNum, teacherName, durationText } = sessionFields;
    ...
    return `
        <div style="text-align:center;border-bottom:2px solid #006655;padding-bottom:10px;margin-bottom:15px;">
            ...
            <h2 style="margin:0;">تقرير حصة القرآن الكريم</h2>
        </div>
        <table style="width:100%;border-collapse:collapse;margin-bottom:15px;font-size:13px;">
            <tr>
                ...
                ${halaNum ? `<td style="padding:6px;border:1px solid #ddd;"><strong>رقم الحلقة:</strong> ${escapeHtml(halaNum)}</td>` : ''}
            </tr>
            <tr>
                <td style="padding:6px;border:1px solid #ddd;"><strong>المدة:</strong> ${escapeHtml(durationText)}</td>
                <td style="padding:6px;border:1px solid #ddd;" ${!halaNum ? 'colspan="2"' : ''}><strong>عدد الطلاب:</strong> ${studentsList.length}</td>
            </tr>
        </table>
        ${studentRows}
        <div style="text-align:center;margin-top:15px;font-size:12px;color:#777;">نسأل الله للطلاب التوفيق والسداد • تقرير حصة القرآن الكريم</div>
    `;
}
...
function exportPDF() {
    ...
    const fname = `تقرير_حصة_القرآن_${(getFieldValue('reportDate') || 'يومي').replace(/[ /\\:]/g, '_')}.pdf`;
    ...
    if (typeof html2pdf !== 'undefined') {
        html2pdf()
            ...
            .then(() => box.remove())
            .catch((err) => {
                console.warn('PDF export failed, fallback to print:', err);
                box.remove();
                window.print();
            });
    } else {
